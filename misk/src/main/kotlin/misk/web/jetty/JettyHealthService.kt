 ...
 * needs to start and stop last to continue responding until the entire service is shutdown.
 *
 * As this is not easily modelled in the service dependency graph, JettyHealthService is managed externally to the graph
 * by starting asynchronously before other services begin starting and terminating after other services have gracefully
 * shut down.
 */
@Singleton
internal class JettyHealthService
@Inject
internal constructor(
  ...
  private val enabled by lazy { webConfig.jettyHealthServiceEnabled(logger) }
    ...
    val threadPool =
      ExecutorThreadPool(
          ThreadPoolExecutor(
            ...
            /* threadFactory = */ ThreadFactoryBuilder().setNameFormat("jetty-health-%d").build(),
          )
        )
        .apply { name = "jetty-health" }
  ...
  internal val healthServerUrl: HttpUrl? by lazy {
    if (!enabled) {
      ...
      server.connectors.mapNotNull { it as? NetworkConnector }.first { it.name == "health" }.toHttpUrl()
    }
  }
  ...
  override fun startUp() {
    ...
    measureTimeMillis {
      ...
      .also { logger.info("Started Jetty Health in ${it.milliseconds} on port ${webConfig.health_port}") }
  }
    ...
    server.addConnector(
      ServerConnector(
        ...
        .apply {
          ...
          webConfig.override_shutdown_idle_timeout?.let { shutdownIdleTimeout = it }
          reuseAddress = true
          name = "health"
          webConfig.host?.let { host = it }
          addBean(connectionMetricsCollector.newConnectionListener(protocol = "http", port = webConfig.health_port))
        }
    )
  }
  ...
  override fun shutDown() {
    ...
    measureTimeMillis { server.stop() }.also { logger.info { "Stopped Jetty Health in ${it.milliseconds}" } }
  }
